 ...
 */
public class VaultClientOld {
	private final static Logger logger = LogManager
			.getLogger(VaultClientOld.class.getName());
	...
	CloudsHandlerOld cloudsHandler;
	...
	public VaultClientOld(String vaultPath, String configPath) {
		...
		for (int i=0; i < cloudcounter; i++) {
			...
			if(Integer.parseInt(status) == 1) {
				...
				try {
					switch (type) {
					case "Dropbox":
						clouds.add(new Dropbox(code, proxy));
						break;
					case "GoogleDrive":
						clouds.add(new GoogleDrive(proxy, index++));
						break;
					case "OneDrive":
						clouds.add(new FolderCloud(code));
						break;
					case "Box":
						try {
		...
		cloudsHandler = new CloudsHandlerOld(clouds, configPath);
